...
# Fungsi untuk memvalidasi cookie
...
def save_cookies(cookies):
    valid_cookies = [cookie for cookie in cookies if is_valid_cookie(cookie)]
    with open(COOKIE_FILE, 'w') as file:
        for cookie in valid_cookies:
            file.write(f"{cookie}\n")
    print(colored(f"{len(valid_cookies)} cookies have been saved to {COOKIE_FILE}", "green"))
...
def auto_connect_adb(ports):
    for port in ports:
        ...
        if result.returncode == 0:
            enable_adb_root_for_all([port])
        else:
            print(f"Failed to connect to port {port}: {result.stderr}")

# Fungsi untuk login menggunakan cookie dengan metode yang lebih komprehensif
def login_with_cookie(device_id, cookie):
    try:
        print(colored(f"Attempting to login to emulator {device_id} with cookie...", "yellow"))
        
        # 1. Stop Roblox jika sedang berjalan
        ...
        # 3. Buat direktori shared_prefs jika belum ada
        subprocess.run([ADB_PATH, '-s', f'127.0.0.1:{device_id}', 'shell', 'mkdir', '-p', 
                       '/data/data/com.roblox.client/shared_prefs'],
                     stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        
        # 4. Buat file XML untuk cookie
        temp_file = "temp_roblox_cookie.xml"
        with open(temp_file, 'w', encoding='utf-8') as f:
            f.write(f'''<?xml version='1.0' encoding='utf-8' standalone='yes' ?>
<map>
    <string name="ROBLOX_COOKIE">{cookie}</string>
</map>''')
        
        # 5. Push file cookie ke device
        subprocess.run([ADB_PATH, '-s', f'127.0.0.1:{device_id}', 'push', temp_file, 
                       '/data/data/com.roblox.client/shared_prefs/com.roblox.client.v2.playerprefs.xml'],
        ...
        # 7. Bersihkan file temporary
        os.remove(temp_file)
        
        # 8. Start Roblox untuk memastikan cookie diterapkan
        ...
        print(colored(f"Successfully logged in to emulator {device_id} with cookie", "green"))
        return True
    except Exception as e:
        print(colored(f"Failed to login to emulator {device_id}: {e}", "red"))
        return False
...
def start_private_server(device_id, private_link):
    try:
        subprocess.run(
            ...
            stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        time.sleep(10)
        subprocess.run([ADB_PATH, '-s', f'127.0.0.1:{device_id}', 'shell', 'am', 'start', '-n', 
                       'com.roblox.client/com.roblox.client.ActivityProtocolLaunch', '-d', private_link],
                       stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        time.sleep(8)                               
...
def start_default_server(device_id, game_id):
    try:
        ...
        time.sleep(10)
        subprocess.run([ADB_PATH, '-s', f'127.0.0.1:{device_id}', 'shell', 'am', 'start', '-n', 
                       'com.roblox.client/com.roblox.client.ActivityProtocolLaunch', '-d', game_url],
                       stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        time.sleep(10)
...
def menu():
    ...
    while True:
        ...
        if choice == '1':
            ...
            new_ports = [port.strip() for port in new_ports if port.strip()]
            if new_ports:
            ...
            if code:
                ...
                print(colored("No private code entered.", "red"))
        elif choice == '5':
            ...
            if instance in ports:
                ...
                print(colored(f"Port {instance} is not in your configured ports.", "red"))
        elif choice == '6':
            while True:
                print("\nCookie Management:")
                print("1. Add new cookies")
                print("2. View all cookies")
                print("3. Delete all cookies")
                print("4. Login all instances with cookies")
                print("5. Login specific instance with cookie")
                print("6. Back to main menu")
                
                cookie_choice = input("Select option (1/2/3/4/5/6): ")
                ...
                if cookie_choice == '1':
                    ...
                    while True:
                        ...
                        if line.strip() == "" and new_cookies:
                            break
                        if is_valid_cookie(line.strip()):
                            new_cookies.append(line.strip())
                        elif line.strip():
                            print(colored("Invalid cookie format (must start with _|WARNING:-DO-NOT-SHARE-THIS.--)", "red"))
                    
                    if new_cookies:
                        existing_cookies = load_cookies()
                        updated_cookies = existing_cookies + new_cookies
                        cookies = save_cookies(updated_cookies)
                        print(colored(f"Added {len(new_cookies)} new cookies. Total cookies now: {len(cookies)}", "green"))
                    else:
                    ...
                    if os.path.exists(COOKIE_FILE):
                        ...
                        cookies = []
                        print(colored("All cookies have been deleted.", "green"))
                    else:
                        print(colored("No cookie file found.", "yellow"))
                        
                elif cookie_choice == '4':
                    if not ports:
                        print(colored("No ports configured. Please set ADB ports first.", "red"))
                        continue
                    ...
                    # Login dengan cookie untuk setiap port yang tersedia
