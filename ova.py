...
def start_instance_in_thread(ports, game_id, private_codes, status):
    ...
    # Jalankan auto_join_game untuk setiap port
    for port in ports:
        thread = threading.Thread(target=auto_join_game, args=(port, game_id, private_codes.get(port), status))
        thread.start()
        threads.append(thread)

    # Jalankan satu thread untuk memantau semua emulator
    monitor_thread = threading.Thread(target=ensure_roblox_running_with_interval, args=(ports, game_id, private_codes, 1))
    monitor_thread.daemon = True  # Agar thread mati saat program berhenti
