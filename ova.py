...
def get_username_from_prefs(device_id):
    ...
    adb_command = [ADB_PATH, "-s", f"emulator-{device_id}", "shell", "cat", "/data/data/com.roblox.client/shared_prefs/prefs.xml"]
...
def start_private_server(device_id, private_link):
    try:
        subprocess.run(
            [ADB_PATH, '-s', f"emulator-{device_id}", 'shell', 'am', 'start', '-n', 'com.roblox.client/com.roblox.client.startup.ActivitySplash', '-d', private_link],
            stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        time.sleep(10)
        # Membuka game di Roblox setelah ActivitySplash
        subprocess.run([ADB_PATH, '-s', f"emulator-{device_id}", 'shell', 'am', 'start', '-n', 'com.roblox.client/com.roblox.client.ActivityProtocolLaunch', '-d', private_link],
                       stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        time.sleep(8)                               
...
def start_default_server(device_id, game_id):
    try:
        ...
        subprocess.run(
            [ADB_PATH, '-s', f"emulator-{device_id}", 'shell', 'am', 'start', '-n', 'com.roblox.client/com.roblox.client.startup.ActivitySplash', '-d', game_url],
            stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        time.sleep(10)
        # Membuka game di Roblox setelah ActivitySplash
        subprocess.run([ADB_PATH, '-s', f"emulator-{device_id}", 'shell', 'am', 'start', '-n', 'com.roblox.client/com.roblox.client.ActivityProtocolLaunch', '-d', game_url],
                       stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        time.sleep(10)
...
def check_roblox_running(device_id):
    try:
        result = subprocess.run(
            [ADB_PATH, '-s', f"emulator-{device_id}", 'shell', 'pidof', 'com.roblox.client'],
...
def force_close_roblox(device_id):
    subprocess.run([ADB_PATH, '-s', f"emulator-{device_id}", 'shell', 'am', 'force-stop', 'com.roblox.client'],
                   stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)               
    time.sleep(8)
