...
# Fungsi untuk mendapatkan username dari prefs.xml (dengan caching)
def get_cached_username(device_id):
    current_time = time.time()
    if device_id in username_cache:
        cached_username, timestamp = username_cache[device_id]
        if current_time - timestamp < cache_expiry_time:
            return cached_username  # Gunakan username dari cache jika belum kedaluwarsa
    
    # Jika tidak ada cache atau sudah kedaluwarsa, ambil username baru
    username = get_username_from_prefs(device_id)
    if username:
        username_cache[device_id] = (username, current_time)
    return username
...
def update_table(status):
    ...
    for device_id, game_status in status.items():
        username = get_cached_username(device_id)  # Mendapatkan username dari prefs.xml
